    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 820, width: "100%" }}>
        <div className="modal-header">
          <span className="modal-title">🛡️ Historial de Moderaciones ({items.length})</span>
          <button className="modal-close" onClick={onClose}>×</button>
        </div>

        {/* Filtros */}
        <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
          <input className="form-input" style={{ maxWidth: 200 }} placeholder="🔍 Buscar en reseña..."
        ...
        {loading ? <Loading /> : filtered.length === 0 ? (
          <div className="empty-state"><div className="empty-state-icon">📋</div><div className="empty-state-text">Sin resultados</div></div>
        ) : (
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 700, width: "100%" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
          <input className="form-input" style={{ maxWidth: 260 }} placeholder="🔍 Buscar en comentario o reseña..."
            value={busqueda} onChange={e => setBusqueda(e.target.value)} />
          <span style={{ alignSelf: "center", fontSize: 12, color: "var(--text-muted)", marginLeft: "auto" }}>
            {filtered.length} resultado{filtered.length !== 1 ? "s" : ""}
          </span>
